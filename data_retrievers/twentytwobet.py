...
async def twentytwobet_tennis_win_match():
    ...
    for event in result_events:
        event_data = {
            ...
            'url': 'https://22wingo.com/line/tennis/' + format_url(event)
        }
...
async def twentytwobet_american_football():
    ...
    for event in result_events:
        event_data = {
            ...
            'start_time': str(convert_time(event['S'])),
            'start_time_ms': event['S'] * 1000,
            'url': 'https://22wingo.com/line/basketball/' + format_url(event)
        }
...
async def twentytwobet_volley():
    ...
    for event in result_events:
        event_data = {
            ...
            'start_time': str(convert_time(event['S'])),
            'start_time_ms': event['S'] * 1000,
            'url': 'https://22wingo.com/line/volleyball/' + format_url(event)
        }
...
async def twentytwobet_football():
    ...
    # result = requests.get(
    #    "https://22wingo.com/LineFeed/Get1x2_VZip?sports=1&count=300&lng=pt&tf=3000000&tz=1&mode=4&country=148"
    ...
    for event in result_events:
        event_data = {
            ...
            'start_time': str(convert_time(event['S'])),
            'start_time_ms': event['S'] * 1000,
            'url': 'https://22wingo.com/line/football/' + format_url(event)
        }
...
def get_competition_ids(sport_arg):
    ...
    comps_url = (
        f"https://22wingo.com/LineFeed/GetSportsShortZip?sports={sport_id}&lng=pt&tf=2880&country=148&partner=151"
...
async def get_events_from_competitions(competition_ids, sport):
    ...
    url_template = ("https://22wingo.com/LineFeed/Get1x2_VZip?sports={sport_id}&champs={"
